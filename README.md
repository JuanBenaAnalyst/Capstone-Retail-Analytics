...
**Purpose:** Educational capstone showing an end-to-end **data-driven retail analytics** pipeline and communicating findings to non-technical stakeholders.
...
- **High-Value segment:** ~**49%** of customers contribute **~74%** of revenue → prioritize **retention** and **stockout prevention**.  
...
- Tableau turns the analysis into an **interactive** product for executives.

**How the story unfolds**
I first make the data trustworthy (types, joins, integrity). I then explore patterns (KPIs, seasonality, category mix), **segment** customers with RFM + K-Means, and **forecast** near-term sales to support inventory planning. Finally, I publish an **Executive dashboard** so stakeholders can explore the results.
<!-- /SECTION:INTRO -->

<!-- SECTION:METHODS -->
## Methods

This section covers data sources, cleaning, features, models, validation, and how to reproduce the results.
...
- **Derived (educational):** `products_with_category_v7_clean.csv` — curated by joining the original `products` and `categories` tables and standardizing labels (merging synonyms, fixing typos, consolidating classes). It improves category granularity but **may contain classification errors**, so treat it as a best-effort educational artifact.
...
> All KPIs and visuals in this README were computed on the **full** `sales_enriched.csv`. If you only use the mini feed, **minor discrepancies** may appear. To reproduce the exact figures, run **NB01** locally to regenerate the full file.

### Feature engineering (NB03)
- **Transaction definition:** one **order** per `SalesID`. RFM window: **2018-01-01 00:00:00 → 2018-05-09 00:00:00**, anchor `t₀ = max(valid SalesDate)`.  
- **RFM:** `recency_days` (days since last purchase), `frequency` (#orders), `monetary` (total spend).  
- **Extra customer features:** `avg_ticket`, `pct_discounted` (share of orders with ≥1 discounted item), `n_categories`. Geography modes (`top_city`, `top_country`) are reporting-only.  
...
**Labeling:** the **High-Value** cluster maximizes `frequency + monetary − recency`. I also publish a binary `hv01` (0=High-Value, 1=Mid/Low).
...
**Stability.** Bootstrap agreement/ARI/NMI is ~0.99. I benchmarked GMM, Agglomerative, and DBSCAN; I kept K-Means for clarity and ease of use.
...
I built a reproducible pipeline from raw multi-table data (**NB01**) to **EDA** (**NB02**), **feature engineering** (**NB03**), **customer segmentation** (**NB04**), and a **short-term forecaster** (**NB05**). Artifacts live under `clean/*`, and the dashboard makes the insights easy to explore.

**Key takeaways**
- **Concentration:** ~**55.9%** of customers drive **80%** of sales → segmentation matters.  
- **High-Value:** **~49%** of customers deliver **~74%** of revenue → protect retention; avoid stockouts.  
- **Forecast uplift:** RF improves over seasonal naïve (CV ≈ **15%**, holdout ≈ **34%**) with **~86%** coverage for 80% PIs.

**Recommendations**
1) **Inventory:** set **min/max** with the forecast and PIs; give perishables and **High-Value** demand special attention.  
2) **Promotions:** activate **Mid/Low** customers with adjacent-category bundles; monitor weekly lift.  
3) **Pricing:** trial **dynamic markdowns** on overstock (wide PIs).  
